...
@section('title', 'Plan kont – ' . config('app.name'))
...
<div class="page-content page-content--wide">
    <div class="page-header">
        ...
        <div class="page-actions">
            <a href="{{ route('plan-kont.create') }}" class="btn btn-primary">Dodaj</a>
            <a href="{{ route('plan-kont.pobierz-csv') }}" class="btn btn-outline">Pobierz CSV</a>
            <form action="{{ route('plan-kont.przyjmij-grupy') }}" method="POST" class="inline-form">
                @csrf
                <button type="submit" class="btn btn-outline">Przypisz grupy</button>
            </form>
            <form id="form-usun-wszystko" action="{{ route('plan-kont.destroy-all') }}" method="POST" class="inline-form">
                @csrf
    ...
    @if($items->isEmpty())
        <p class="empty-state">Brak pozycji w planie kont. <a href="{{ route('plan-kont.create') }}">Dodaj pierwszą pozycję</a>.</p>
    @else
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->grupa }}</td>
                        <td>{{ $item->nazwa }}</td>
                        <td>{{ $item->rodzaj_pozycji }}</td>
                        <td class="col-actions">
                            <a href="{{ route('plan-kont.edit', $item) }}" class="btn-icon" title="Edytuj">
                            ...
                            <form action="{{ route('plan-kont.destroy', $item) }}" method="POST" class="inline-form" onsubmit="return confirm('Usunąć tę pozycję?');">
                                @csrf
                                @method('DELETE')
